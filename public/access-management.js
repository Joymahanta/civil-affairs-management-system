(() => {
  ...
  const err = (f, m = '') => { const n = $('.error', f); n.textContent = m; n.classList.toggle('hidden', !m) };
  ...
  function initGlobalSearch() {
    ...
    const navigate = page => {
      ...
      const button = [...document.querySelectorAll('[data-admin-page]')]
        .find(node => node.dataset.adminPage === page);
  ...
  async function boot() {
    const session = await api('/api/auth/session'); if (!session.authenticated) return; document.querySelectorAll('[data-close]').forEach(b => b.onclick = () => modal(b.dataset.close, false)); await Promise.all([loadDesignations(), loadStaff()]);
    initGlobalSearch();
