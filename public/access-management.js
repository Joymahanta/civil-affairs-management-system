(() => {
  ...
  function initGlobalSearch() {
    ...
    const navigate = page => { hide(); if (!page || typeof page !== 'string') return; const button = [...document.querySelectorAll('[data-admin-page]')].find(node => node.dataset.adminPage === page); if (button) button.click(); };
    const render = (items, query) => { if (!items.length) { results.innerHTML = `<div style="padding:14px;color:#687b76;font-size:14px">No matches for “${esc(query)}”.</div>`; results.style.display = 'block'; return; } results.innerHTML = items.slice(0, 20).map((item, index) => `<button type="button" data-search-index="${index}" style="display:block;width:100%;text-align:left;border:0;background:transparent;border-radius:9px;padding:10px 12px;cursor:pointer"><span style="display:block;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#148468;margin-bottom:3px">${esc(item.label)}</span><b style="display:block;color:#102d27;font-size:14px">${esc(item.title)}</b><span style="display:block;color:#687b76;font-size:13px;margin-top:2px">${esc(item.detail)}</span></button>`).join(''); results.style.display = 'block'; results.querySelectorAll('[data-search-index]').forEach(button => button.addEventListener('mouseenter', () => button.style.background = '#f3f7f5')); results.querySelectorAll('[data-search-index]').forEach(button => button.addEventListener('mouseleave', () => button.style.background = 'transparent')); results.querySelectorAll('[data-search-index]').forEach(button => button.addEventListener('click', () => navigate(items[Number(button.dataset.searchIndex)].page))); };
    input.addEventListener('input', () => { clearTimeout(timer); const query = input.value.trim(); if (!query) { hide(); return; } timer = setTimeout(async () => { const currentRequest = ++requestId; try { const records = await Promise.all(sources.map(async source => ({ source, rows: await source.get() }))); if (currentRequest !== requestId) return; const q = query.toLowerCase(), matches = []; records.forEach(({ source, rows }) => rows.forEach(row => { if (!source.fields.some(field => String(row[field] ?? '').toLowerCase().includes(q))) return; let title = row[source.key] || row.name || row.reference || row.asset_code || row.tender_no, detail = ''; if (source.label === 'Employee') detail = `${row.designation || 'Unassigned'} · ${row.department || ''} · ${row.phone || ''}`; if (source.label === 'Case') detail = `${row.reference} · ${row.category || ''} · ${row.location || ''} · ${row.status || ''}`; if (source.label === 'Equipment') detail = `${row.asset_code} · ${row.holder || 'Store'} · ${row.status || ''}`; if (source.label === 'Tender') detail = `${row.tender_no} · ${row.scope || ''} · ${row.status || ''}`; matches.push({ label: source.label, page: source.page, title, detail }); })); render(matches, query); } catch (e) { results.innerHTML = '<div style="padding:14px;color:#b42318;font-size:14px">Search could not be completed.</div>'; results.style.display = 'block'; } }, 220); });
  ...
  async function boot() {
    const session = await api('/api/auth/session'); if (!session.authenticated) return;
    document.querySelectorAll('[data-close]').forEach(b => b.onclick = () => modal(b.dataset.close, false));
    await Promise.all([loadDesignations(), loadStaff()]);
    initGlobalSearch();
