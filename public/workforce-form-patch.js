(() => {
  ...
  function modalShell(id, title, subtitle, bodyHtml) {
    let backdrop = document.getElementById(id); if (backdrop) return backdrop;
    backdrop = document.createElement('div'); backdrop.className = 'modal-backdrop'; backdrop.id = id;
    backdrop.innerHTML = `<form class="modal" id="${id}-form"><button type="button" class="close" data-local-close>×</button><h2>${esc(title)}</h2><p class="sub">${esc(subtitle)}</p>${bodyHtml}</form>`; document.body.appendChild(backdrop);
    backdrop.addEventListener('click', event => { if (event.target === backdrop || event.target.closest('[data-local-close]')) backdrop.classList.remove('show'); }); return backdrop;
  }
  ...
  async function openDepartmentModal() {
    ...
    const modal = modalShell('department-maker-modal', 'Make department', 'Create the departments that will appear in staff and designation dropdowns.', body); const form = modal.querySelector('form');
    if (form.dataset.bound !== '1') { form.dataset.bound = '1'; form.addEventListener('submit', async event => { event.preventDefault(); const error = form.querySelector('.error'); error.classList.add('hidden'); error.textContent = ''; try { const values = Object.fromEntries(new FormData(form)); await getJson('/api/departments', { method:'POST', body:JSON.stringify(values) }); form.reset(); await loadDepartments(true); await loadDesignations(true); await refreshStaffDropdowns(); renderDepartmentList(); toastMessage('Department created.'); } catch (errorValue) { error.textContent = errorValue.message; error.classList.remove('hidden'); } }); }
    await loadDepartments(true); renderDepartmentList(); openLocalModal('department-maker-modal');
  }
  function renderDepartmentList() { const list = document.getElementById('department-list'); if (!list) return; list.innerHTML = departments.map(item => `<div class="activity"><span class="activity-dot">▦</span><p><b>${esc(item.name)}</b><br><span class="sub">${Number(item.designation_count || 0)} designation${Number(item.designation_count || 0) === 1 ? '' : 's'}</span></p></div>`).join('') || '<p class="empty">No departments yet.</p>'; }
  async function openDesignationModal() {
    ...
    const modal = modalShell('designation-maker-modal', 'Make designation', 'Create a designation under a specific department. Example: Electrical → Electrical Engineer, Electrical Apprentice.', body); const form = modal.querySelector('form'); const department = form.querySelector('[name="departmentId"]');
    await loadDepartments(true); setOptions(department, departments, 'Select department', 'id', 'name');
    if (form.dataset.bound !== '1') { form.dataset.bound = '1'; form.addEventListener('submit', async event => { event.preventDefault(); const error = form.querySelector('.error'); error.classList.add('hidden'); error.textContent = ''; try { const values = Object.fromEntries(new FormData(form)); await getJson('/api/designations', { method:'POST', body:JSON.stringify(values) }); form.reset(); await loadDesignations(true); await loadDepartments(true); await refreshStaffDropdowns(); renderDesignationList(); toastMessage('Designation created.'); } catch (errorValue) { error.textContent = errorValue.message; error.classList.remove('hidden'); } }); }
    await loadDesignations(true); renderDesignationList(); openLocalModal('designation-maker-modal');
  }
  function renderDesignationList() { const list = document.getElementById('designation-list'); if (!list) return; const groups = departments.map(department => ({ department, rows: designations.filter(item => Number(item.department_id) === Number(department.id)) })).filter(group => group.rows.length); list.innerHTML = groups.map(group => `<div class="card" style="padding:12px;margin-bottom:10px"><b>${esc(group.department.name)}</b>${group.rows.map(item => `<div class="activity" style="padding:8px 0"><span class="activity-dot">♧</span><p><b>${esc(item.name)}</b><br><span class="sub">${Number(item.staff_count || 0)} staff</span></p></div>`).join('')}</div>`).join('') || '<p class="empty">No designations yet.</p>'; }
  async function refreshStaffDropdowns() { await Promise.all([loadDepartments(true), loadDesignations(true)]); const add = document.getElementById('add-staff-form'); if (add) { const department = add.querySelector('[name="department"]'); const designation = add.querySelector('[name="designationId"]'); const selectedDepartment = department?.value || ''; const selectedDesignation = designation?.value || ''; await populateDepartmentSelect(department, selectedDepartment); await populateDesignationSelect(designation, selectedDepartment, selectedDesignation); } }
  function toastMessage(message) { if (typeof window.toast === 'function') window.toast(message); else console.info(message); }
  function bindMakerButtons() { const departmentButton = document.getElementById('make-department'); const designationButton = document.getElementById('make-designation'); if (departmentButton && departmentButton.dataset.makerBound !== '1') { departmentButton.textContent = 'Make department'; departmentButton.dataset.makerBound = '1'; departmentButton.addEventListener('click', () => openDepartmentModal().catch(error => toastMessage(error.message))); } if (designationButton && designationButton.dataset.makerBound !== '1') { designationButton.textContent = 'Make designation'; designationButton.dataset.makerBound = '1'; designationButton.addEventListener('click', () => openDesignationModal().catch(error => toastMessage(error.message))); } }
  function repair() { bindMakerButtons(); repairEditForm(); const add = document.getElementById('add-staff-form'); if (add && add.dataset.departmentDesignationReady !== '1') { add.dataset.departmentDesignationReady = '1'; repairAddForm().catch(error => console.error('[workforce-form]', error)); } }
