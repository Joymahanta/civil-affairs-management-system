...
function initAdminConsole() {
  ...
  async function loadStaff() {
    ...
    $('#staff-body').innerHTML = state.staff.map(item => `<tr><td><b>${escapeHtml(item.name)}</b></td><td>${escapeHtml(item.department)}</td><td>${badge(item.attendance === 'Present' ? 'Available' : 'Review')}</td><td>${escapeHtml(item.current_task || '—')}</td><td>${item.last_sms_at ? formatDate(item.last_sms_at) : '—'}</td><td><button class="button secondary small" data-edit-staff="${item.id}">Update</button></td></tr>`).join('');
