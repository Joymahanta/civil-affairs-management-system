(() => {
  ...
  function improveQuickActionIcons() {
    ...
    style.textContent = `
      ...
      .quick-actions .quick-icon { display:grid !important; place-items:center; width:42px !important; height:42px !important; min-width:42px; margin-bottom:10px; border-radius:11px; background:#e5f5ee; color:#147a5d; font-size:25px !important; line-height:1; font-weight:700; }
      ...
      @media (max-width:760px) { .quick-actions .card { grid-template-columns:1fr 1fr; } .quick-actions .quick-action { min-height:116px; } }
    `;
    document.head.appendChild(style);
  }

  function loadQrScanner() {
    if (document.querySelector('script[src="/qr-scanner.js"]')) return;
  ...
  function bind() {
    ...
    loadQrScanner();
    document.querySelectorAll('[data-report-type]').forEach(button => button.addEventListener('click', resetComposer, true));
    document.querySelectorAll('#report-success [data-close="report-modal"]').forEach(button => button.addEventListener('click', resetComposer));
    document.getElementById('report-modal')?.addEventListener('click', event => { if (event.target.id === 'report-modal') resetComposer(); });
  }
